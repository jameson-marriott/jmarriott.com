...
I use the [Hugo Coder](https://github.com/luizdepra/hugo-coder) theme for this website, but I don't like the default favicon, so I decided to make a new one using ggplot2. For those of you who don't know, a favicon is the little icon that appears on the browser tab next to the website name.
...
```{r favicon, fig.height=3, fig.width=3, message=FALSE, warning=FALSE, dpi=300}
...
<img src="images/favicon.svg" width="25%" height="auto"/>
...
The svg image looks way better on my high-resolution screen, and I also figured out that svgs are human-readable, which is super cool. But it appears that the most common browsers don't support them, so I'll stick with the rasterized images for my favicon.